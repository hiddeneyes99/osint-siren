...
import { Smartphone, CreditCard, Car, Globe, AlertTriangle, History, ShieldAlert } from "lucide-react";
...
import { useMobileInfo, useAadharInfo, useVehicleInfo, useIpInfo } from "@/hooks/use-services";
import { useAuth } from "@/hooks/use-auth";
import { mobileInfoSchema, aadharInfoSchema, vehicleInfoSchema, ipInfoSchema } from "@shared/schema";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const mutationWithError = mutations.find(m => 
      m.error && (m.error as any).message?.toLowerCase().includes("protected")
    );
  ...
  }, [mobileMutation.error, aadharMutation.error, vehicleMutation.error, ipMutation.error]);
  ...
  // Handlers
  const onMobileSubmit = (data: z.infer<typeof mobileInfoSchema>) => mobileMutation.mutate(data);
  const onAadharSubmit = (data: z.infer<typeof aadharInfoSchema>) => aadharMutation.mutate(data);
  const onVehicleSubmit = (data: z.infer<typeof vehicleInfoSchema>) => vehicleMutation.mutate(data);
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <CyberCard className="max-w-md w-full text-center py-12">
            ...
            <p className="text-muted-foreground mb-8">Authentication required to access system tools.</p>
            <CyberButton className="w-full" onClick={() => setIsAuthModalOpen(true)}>INITIALIZE LOGIN SEQUENCE</CyberButton>
          </CyberCard>
        </div>
      ...
      <main className="flex-1 container px-4 py-4 md:py-8">
        <div className="flex flex-col lg:flex-row gap-6 h-full">
          ...
          <div className="w-full lg:w-64 flex-shrink-0">
            <div className="mb-4">
              <h2 className="text-[10px] md:text-xs font-mono text-primary/60 mb-2 uppercase tracking-widest px-2">Select Module</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex lg:flex-col gap-2 w-full">
              ...
              >
                <Smartphone className="w-3 h-3 md:w-4 md:h-4 mr-1.5 md:mr-3 shrink-0" /> <span className="truncate">Mobile</span>
              </CyberButton>
              ...
              <CyberButton 
                ...
                className="justify-start px-2 md:px-4 py-2 md:py-3 font-mono text-[9px] md:text-sm uppercase transition-all h-auto"
              >
                <CreditCard className="w-3 h-3 md:w-4 md:h-4 mr-1.5 md:mr-3 shrink-0" /> <span className="truncate">Aadhar</span>
              </CyberButton>
              ...
              <CyberButton 
                ...
                className="justify-start px-2 md:px-4 py-2 md:py-3 font-mono text-[9px] md:text-sm uppercase transition-all h-auto"
              >
                <Car className="w-3 h-3 md:w-4 md:h-4 mr-1.5 md:mr-3 shrink-0" /> <span className="truncate">Vehicle</span>
              </CyberButton>
              ...
              <CyberButton 
                ...
                className="justify-start px-2 md:px-4 py-2 md:py-3 font-mono text-[9px] md:text-sm uppercase transition-all h-auto"
              >
                <Globe className="w-3 h-3 md:w-4 md:h-4 mr-1.5 md:mr-3 shrink-0" /> <span className="truncate">IP Probe</span>
              </CyberButton>
              ...
              <CyberButton 
                ...
                className="justify-start px-2 md:px-4 py-2 md:py-3 font-mono text-[9px] md:text-sm uppercase transition-all h-auto"
              >
                <History className="w-3 h-3 md:w-4 md:h-4 mr-1.5 md:mr-3 shrink-0" /> <span className="truncate">History</span>
              </CyberButton>
            </div>

            <div className="mt-4 lg:mt-8 p-3 md:p-4 border border-primary/20 bg-primary/5 rounded-none">
              <h3 className="text-[10px] md:text-xs font-bold text-primary mb-2">SYSTEM STATUS</h3>
                ...
                >
                  <CyberCard>
                    <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6 border-b border-primary/20 pb-3 md:pb-4">
                      ...
                      <h2 className="text-base md:text-xl font-bold font-display tracking-wider">MOBILE NUMBER INTELLIGENCE</h2>
                    </div>
                    
                    <Form {...mobileForm}>
                      <form onSubmit={mobileForm.handleSubmit(onMobileSubmit)} className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end">
                        <FormField
                          control={mobileForm.control}
                          name="number"
                          render={({ field }) => (
                            <FormItem className="flex-1">
                              <FormLabel className="text-primary/80 font-mono text-[10px] md:text-xs">TARGET NUMBER (IN)</FormLabel>
                              <FormControl>
                                <Input 
              ...
              {activeTab === "aadhar" && (
                ...
                >
                  <CyberCard className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h2 className="text-2xl font-bold font-display tracking-widest text-primary mb-2">AADHAR INFO MODULE</h2>
                    <p className="text-primary/60 font-mono text-sm uppercase tracking-widest">Status: [ COMING SOON ]</p>
                ...
                >
                  <CyberCard>
                    <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6 border-b border-primary/20 pb-3 md:pb-4">
                      ...
                      <h2 className="text-base md:text-xl font-bold font-display tracking-wider">VEHICLE RECONNAISSANCE</h2>
                    </div>
                    
                    <Form {...vehicleForm}>
                      <form onSubmit={vehicleForm.handleSubmit(onVehicleSubmit)} className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end">
                        <FormField
                          control={vehicleForm.control}
                          name="number"
                          render={({ field }) => (
                            <FormItem className="flex-1">
                              <FormLabel className="text-primary/80 font-mono text-[10px] md:text-xs">REGISTRATION NUMBER (RC)</FormLabel>
                              <FormControl>
                                <Input 
                ...
                >
                  <CyberCard>
                    <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6 border-b border-primary/20 pb-3 md:pb-4">
                      ...
                      <h2 className="text-base md:text-xl font-bold font-display tracking-wider">NETWORK PROBE</h2>
                    </div>
                    
                    <Form {...ipForm}>
                      <form onSubmit={ipForm.handleSubmit(onIpSubmit)} className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end">
                        <FormField
                          control={ipForm.control}
                          name="ip"
                          render={({ field }) => (
                            <FormItem className="flex-1">
                              <FormLabel className="text-primary/80 font-mono text-[10px] md:text-xs">IP ADDRESS (IPv4)</FormLabel>
                              <FormControl>
                                <Input 
                ...
                >
                  <CyberCard className="flex-1">
                    <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6 border-b border-primary/20 pb-3 md:pb-4">
                      ...
                      <h2 className="text-base md:text-xl font-bold font-display tracking-wider">SEARCH LOG HISTORY</h2>
                    </div>
                    
                    <ScrollArea className="h-[350px] md:h-[400px] pr-2 md:pr-4">
                      <div className="space-y-3 md:space-y-4">
                        {history.length === 0 ? (
                          <p className="text-center text-muted-foreground font-mono text-xs md:text-sm">NO LOGS FOUND</p>
                        ) : (
                          history.map((log: any) => (
                            <div key={log.id} className="border border-primary/20 p-3 md:p-4 bg-primary/5 hover:bg-primary/10 transition-colors">
                              <div className="flex flex-col sm:flex-row justify-between items-start gap-1 mb-2">
                                <span className="text-primary font-bold uppercase text-[10px] md:text-sm">{log.service} MODULE</span>
                                <span className="text-[9px] md:text-xs text-muted-foreground font-mono">{new Date(log.createdAt).toLocaleString()}</span>
                              </div>
                              <p className="font-mono text-[10px] md:text-sm text-primary/80 mb-3 md:mb-4 tracking-tight break-all">QUERY: {log.query}</p>
                              {log.result && (
                                <TerminalOutput 
                                  data={log.result} 
